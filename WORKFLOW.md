...
GitHub操作が利用できる場合、完了またはblockedの要約を現在のIssueへ1回だけ記録し、再実行を止めるため `agent:implement` ラベルを外す。blockedの場合はさらに `agent:blocked` を付ける。Issueはcloseしない。ほかのIssue、PR、ラベルには触れない。
